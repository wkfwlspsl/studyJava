...
public class TestAA {
	...
	public static int[] solution(int[] heights) {
        ...
        for(int sender_idx=heights.length-1; sender_idx>0; sender_idx--) {
        	int sender = stack.pop();
        	int rNum = 0;
        	
        	while(!stack.empty()) {
        		int receiver = stack.pop();
        		rNum++;
        		if(receiver > sender) {
        			answer[sender_idx] = sender_idx - rNum + 1;
        			break;
        		}
        	}
        	
        	for(int i=sender_idx-rNum; i<sender_idx; i++) {
        		stack.push(heights[i]);
        	}
        }
